...
# Login/register - role is never shown or accepted from the form: every self-signup goes
# through `AuthService.register_customer!`, which always requests the `customer` role.
# Seller accounts are provisioned out-of-band (same limitation the reference web app
# documents - there is no self-service "become a seller" flow in this UI by design).
class App < Sinatra::Base
  ...
  post "/login" do
    ...
    begin
      ...
      destination = auth_session.user[:customRole] == "seller" ? "/seller" : consume_return_to
