...
/// every authenticated request in the app. Before this existed, only `SessionStore.bootstrap()`
/// had any refresh-on-expiry behavior, and only once at launch — every other authenticated call
/// (`CollectionsGateway`'s product/order/cart CRUD, used for the whole life of a session) just let
/// a 401 from an access token that expired mid-session propagate as a raw error up to the UI,
/// which is a real gap: a token that expires while someone is mid-checkout or browsing the seller
/// queue would otherwise force a manual sign-out/sign-in instead of transparently refreshing.
/// `CollectionsGateway` routes every `DataAPI` call through `perform(_:)` for exactly this reason.
///
...
/// calls each hit a 401 at nearly the same moment (e.g. a products fetch and a cart fetch racing
/// at token expiry), they must not both redeem the same stored refresh token — the second redeem
/// would fail against an already-rotated token. `inFlightRefresh` de-duplicates that race into a
/// single refresh call that every concurrent 401 awaits together.
actor AccessTokenCoordinator {
    ...
    private func refreshedAccessToken() async throws(ErrorResponse) -> String {
        ...
        guard let authGateway, let tokenStore, let stored = tokenStore.load() else {
            throw ErrorResponse.error(401, nil, nil, MudbaseClientError.missingTokenInLoginResponse)
        }
