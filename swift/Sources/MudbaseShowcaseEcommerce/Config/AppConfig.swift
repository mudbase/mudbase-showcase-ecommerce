...
/// see README "Configuration"). Deliberately NOT declared as an SPM `resources:` entry: that
/// would make `swift build` fail whenever `Config.plist` hasn't been created yet, and this repo's
/// own verification step (`swift build`) must succeed out of the box with no local secrets
/// present. The cost is that config is only checked at runtime, not at compile time — `AppConfig`
/// surfaces that as a typed, recoverable state (`ConfigurationState`) rather than crashing, so the
/// app can show a real "Configuration Required" screen instead of dying at launch.
struct AppConfig: Sendable, Equatable {
    let projectId: String
    let baseURL: URL
    let productsCollectionId: String
    let ordersCollectionId: String
    let cartsCollectionId: String
    let payLinkProxyBaseURL: URL

    private static let defaultBaseURLString = "https://cloud.mudbase.dev"
    private static let defaultProxyURLString = "https://mudbase-showcase-ecommerce.vercel.app"
    ...
    static func load(bundle: Bundle = .main, environment: [String: String] = ProcessInfo.processInfo.environment) -> Result<AppConfig, ConfigurationError> {
        ...
        guard let productsCollectionId = value("ProductsCollectionId", envKey: "MUDBASE_PRODUCTS_COLLECTION_ID") else {
            return .failure(.missingKey("ProductsCollectionId"))
        }
        guard let ordersCollectionId = value("OrdersCollectionId", envKey: "MUDBASE_ORDERS_COLLECTION_ID") else {
            return .failure(.missingKey("OrdersCollectionId"))
        }
        guard let cartsCollectionId = value("CartsCollectionId", envKey: "MUDBASE_CARTS_COLLECTION_ID") else {
            return .failure(.missingKey("CartsCollectionId"))
        }
        let baseURLString = value("MudbaseBaseURL", envKey: "MUDBASE_BASE_URL") ?? defaultBaseURLString
        let proxyURLString = value("PayLinkProxyBaseURL", envKey: "MUDBASE_PAYLINK_PROXY_BASE_URL") ?? defaultProxyURLString

        guard let baseURL = URL(string: baseURLString) else {
            return .failure(.invalidURL("MudbaseBaseURL", baseURLString))
        }
        guard let payLinkProxyBaseURL = URL(string: proxyURLString) else {
            return .failure(.invalidURL("PayLinkProxyBaseURL", proxyURLString))
        }
        ...
        return .success(
            AppConfig(
                ...
                payLinkProxyBaseURL: payLinkProxyBaseURL
            )
        )
    }
...
/// it through every intermediate view's init — used only for read access; view models that need it
/// are still constructed explicitly by their parent view (see README "Architecture") so DI stays
/// visible at each call site rather than hidden behind environment lookups.
