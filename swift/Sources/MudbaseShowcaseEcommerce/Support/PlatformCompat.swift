...
/// target also builds for macOS (see `Package.swift`'s platforms list, needed so `swift build`
/// succeeds from the CLI without Xcode). These no-op on macOS instead of failing to compile.
extension View {
    @ViewBuilder
    ...
    /// `.tabViewStyle(.page)` is unavailable on macOS (page-style paging is an iOS/tvOS/watchOS
    /// concept) — falls back to the platform default `TabView` style there.
    @ViewBuilder
