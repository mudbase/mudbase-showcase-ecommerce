...
/// to a status-code-derived message so no failure ever surfaces a raw `Error` description to a user.
enum MudbaseAPIError {
    ...
    struct DisplayableError: Error, Equatable {
        let statusCode: Int
        let message: String
        /// e.g. "EMAIL_VERIFICATION_REQUIRED" or "KYC_REQUIRED" — callers switch on this when the
        /// UI needs to branch on a specific server-side reason rather than just show text.
        let code: String?
    }
