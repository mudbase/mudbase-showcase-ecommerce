...
/// :collectionId/data`) for the `products`/`orders`/`carts` collections.
///
/// Bypasses the generated `DataApi` wrapper for the same reason as
/// [AuthService]: `DataApi.listData`'s typed response,
/// `DataListResponseDataInner`, only declares `_id`/`createdAt`/`updatedAt`
/// (confirmed in `mudbase-sdk/dart/lib/src/model/
/// data_list_response_data_inner.dart` - its generated deserializer routes
/// every other key into an `unhandled` list that is then discarded). Every
/// product/order/cart field this app actually renders would be silently
/// dropped from a list read. Single-document reads/writes
/// (`getData`/`createData`/`updateData`) type their `data` as a
/// `built_value` `JsonObject`, which *is* fully generic, but this service
/// stays on raw `dio` calls throughout for one consistent, fully-tested code
/// path rather than splitting collection access across two different
/// mechanisms.
